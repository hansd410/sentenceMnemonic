...
class MemoryAnsPointer(nn.Module):
	...
	def forward(self, x, y, x_mask, y_mask, sent_idx_list, ans_sent_idx_list):
		...
		if self.normalize:
			# Paul modified here
			max_n_sent = max([len(sent) for sent in sent_idx_list])
			max_n_word_in_sent = max([max([idx[1]-idx[0] for idx in sent]) for sent in sent_idx_list])

			sent_s = s.new()
			sent_e = e.new()
			sent_s.resize_(s.size(0), max_n_sent, max_n_word_in_sent)
			sent_e.resize_(e.size(0), max_n_sent, max_n_word_in_sent)
			...
			if self.training:
				# In training we output log-softmax for NLL
				p_s = F.log_softmax(sent_s, dim=2)
				p_e = F.log_softmax(sent_e, dim=2)
			else:
				# ...Otherwise 0-1 probabilities
				p_s = F.softmax(sent_s, dim=2)
				p_e = F.softmax(sent_e, dim=2)
		else:
